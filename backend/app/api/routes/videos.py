...
@router.get(
    ...
    description="Return the 5 most recent video generations.",
    response_model=ApiResponse[list[VideoResponse]],
)
async def get_history(
    service: VideoService = Depends(get_video_service),
) -> ApiResponse[list[VideoResponse]]:
    videos = await service.get_history(limit=5)
    responses = []
    for v in videos:
...
async def get_video(
    ...
    video = await service.get_generation(video_id)
    response = VideoResponse.model_validate(video, from_attributes=True)
    return ApiResponse.ok(response)
...
async def delete_video(
    ...
    await service.delete_generation(video_id)
